...
performance. For example, as shown in \reftable{qos_config}, the minimum bandwidwith required by VoIP is 400Kbps and the recommended
...
and even the Open vSwitch (OVS) does not yet support per-flow QoS described in OpenFlow 1.3 specification\todo{citation}. To overcome
...
\begin{algorithm}[htp]
  ...
  \uIf{ $bw + f.minimum < C$ }{
    ...
    $dfs(index+1, score+f.priority*0.6, bw+f.recommend, temp\_assign)$ \;
    $temp\_assign.removeLast()$ \;
  }
