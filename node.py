...
class OVSHtbSwitch( OVSSwitch ):
    """A version of OVSSwitch which you can use with both TCIntf and OVS's QoS
       ...
       disciplines: Hierarchical Token Bucket (HTB)."""
    
    def TCReapply(self, intf ):
        """The general problem here is that Open vSwitch believes it is the only
           software managing the Linux tc queues on this interface. 
           To resolve the conflict, we re-create the user switch's configuration, 
           but as a leaf of the TCIntf-created configuration."""
        if isinstance( intf, TCIntf ):
            ...
            intf.tc( "%s class add dev %s classid 1:0xfffe parent 1: htb rate "
                     + ifspeed )
